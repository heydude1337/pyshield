...
#class config_and_data_manager:
#  _default_file = join(__pkg_root__, 'config.yml')
#
...
#      self.config = None
#  
#  def update(config_file = _default_file):
#    if not(isfile(config_file)):
#      log.error(FILE_ERROR.format(config_file))
#      if not(config_file == self._default_file):
#        log.info('Try reading config from default config file instead: ' + self._default_file)
#        self.update(config_file = self._default_file)
#      else:
#        print('Cannot read config, see log file for details')
#        raise
#    self.config = self._read_config(congig_file)
#  def _read_config(config_file):
#    config = yaml_wrap.read_yaml(config_file)
#    return config
#    
#    s
#
